...
# if working directory is an empty string, currenDirectory prepend for files is blank, i.e. default value
if ( ( fileExists == 0 ) & ( length(myDirectory$dir_name ) = 0) )  {currentDirectory <- paste("",sep="")

                                        #
# if workingDirector file exists, override the workign directory using contents
# of set_myWorkingDirectory.txt


# we need the current directory path to store files
currentDirectory <- getwd()
# we will need the  present date for file names
...
dbGetQuery(con, paste("
   ...
   SELECT time_log_fact_key,         project_key,                       person_key,                         team_key,
    ...
    WHERE   time_log_fact_hist.row_current_flag  = 1
