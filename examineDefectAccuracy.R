...
examineDefectAccuracy <- function(DF)
{
  ...
  if (nrow(DF_temp_nullddid) > 0) {
    #DF_nullddid <- data.frame(key=as.character(DF_temp_nullddid[,"defect_log_fact_key"]),column="defect_fix_defect_identifier",data=as.character(DF_temp_nullddid[,"defect_fix_defect_identifier"]),data_quality="null defect fix defect identifier")
    DF_nullddid <- data.frame(key=as.character(DF_temp_nullddid[,"defect_log_fact_key"]),column="defect_fix_defect_identifier",data="null",data_quality="null data")
  } else {
    DF_nullddid <- data.frame(key="defect_log_fact_key",column="defect_fix_defect_identifier",data="no data",data_quality="no null datar")
  }
