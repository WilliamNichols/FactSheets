...
getDQDataFrame <- function(unit, DF_list, selection_flgs, currentDirectory, unit_name) {
  ...
  for (element in unit) {
    ...
    if (nrow(time_info) == 0) {
      ...
      # Get the interruption data for time log
    ...
    if (nrow(time_overlap_info) == 0) {
      ...
      DF_overlap_temp$difference <- as.POSIXct(DF_overlap_temp$pre_time_log_end_date) - as.POSIXct(DF_overlap_temp$time_log_start_date)
    ...
    if (nrow(defect_info) == 0) {
      ...
      DF_accuracy_defect <- examineDefectAccuracy(defect_info)
      write.csv(DF_accuracy_defect, "DF_accuracy_defect.csv", row.names=F)
    ...
    DF_dq_temp <- data.frame(element=element, number_of_time_records=num_time_records, benford_score_in_time=ben_score_time, number_of_interrupt_in_time=num_interrupt, number_of_inaccuracy_records_in_time=length_inaccuracy_time, number_of_null_in_time=num_null_delta, number_of_zero_in_time=num_zero_delta,number_of_negative_in_time=num_negative_delta, number_of_future_date_in_time=num_future_time, number_of_illedal_year_in_time=num_illyear_time, number_of_illegal_month_in_time=num_illmonth_time, number_of_illegal_day_in_time=num_illday_time, number_of_illegal_hour_in_time=num_illhour_time, number_of_illegal_minute_in_time=num_illminute_time, number_of_illegal_second_in_time=num_illsecond_time, number_of_overlap_records=num_overlap, total_overlap_time=total_overlap, number_of_defect_records=num_defect_records, benford_score_in_defect=ben_score_deftime, number_of_inaccuracy_records_in_defect=length_inaccuracy_defect, number_of_zero_in_deffixtime=num_zero_deftime, number_of_null_in_defect=num_null_deftime, number_of_negative_in_deffixtime=num_negative_deftime, number_of_future_date_in_defect=num_future_deftime, number_of_illegal_year_in_defect=num_illyear_deftime, number_of_illegal_month_in_defect=num_illmonth_deftime, number_of_illegal_day_in_defect=num_illday_deftime, number_of_illegal_hour_in_defect=num_illhour_deftime, number_of_illegal_minute_in_defect=num_illminute_deftime, number_of_illegal_second_in_defect=num_illsecond_deftime, number_of_size_records=num_size_records, number_of_inaccuracy_records_in_size=length_inaccuracy_size, number_of_null_in_size=num_null_size, number_of_negative_in_size=num_negative_size, number_of_no_plan_records_in_size=num_no_plan_size, number_of_no_actual_records_in_size=num_no_actual_size, number_of_negative_in_task=num_negative_task, number_of_future_date_in_task=num_future_task, number_of_illegal_year_in_task=num_illyear_task, number_of_illegal_month_in_task=num_illmonth_task, number_of_illegal_day_in_task=num_illday_task, number_of_illegal_hour_in_task=num_illhour_task, number_of_illegal_minute_in_task=num_illminute_task, number_of_illegal_second_in_task=num_illsecond_task, number_of_no_start_date_in_task=num_no_start_task, number_of_no_completion_date_in_task=num_no_completion_task)
