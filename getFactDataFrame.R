...
# 2014/7/17
# Yasutaka Shirai
# reviewed 2015/05.01 WRN
# includes
#  plan_duration, actual_duration (in net work days)
#
...
getFactDataFrame <- function(unit, DF_list, selection_flgs, currentDirectory, unit_name) {
  ...
  tab_bcws_info     <- DF_list$tab_bcws_info
  ...
  tab_task_completion_info <- DF_list$tab_task_completion_info
  ...
  # depending upon the scope (project/component)
  # set the file name to project/component fact_sheet__date.csv
  ...
  #create the output file for the (component_or_Project)_fact_sheet
  ...
  for (element in unit) {
    ...
    organization_name <- NoData

    ## Extract team and individuals information
    if (length(tab_teams_info) == 0) {
      if (unit_name == "project") {
        team_name <- NoData
        team_size <- NoData
        individuals <- NoData
    ...
    # plan deviation of duration
    plan_duration_deviation <- actual_duration - plan_duration

    # % Deviation of duration
    duration_deviation_ratio <- plan_duration_deviation/plan_duration
    ...
    if (length(tab_ev_info) == 0) {
      ...
      ev_info <- subset(tab_ev_info, get(unit_key)==element)
      ...
      BAC  <- sum(ev_plan_info$task_plan_time_minutes, na.rm=TRUE)/60
      BCWP <- sum(ev_plan_complete_info$task_plan_time_minutes, na.rm=TRUE)/60
      ACWP <- sum(ev_actual_complete_info$task_actual_time_minutes, na.rm=TRUE)/60
    }
    ...
    # Calculate effort deviation
    plan_effort_deviation <- ACWP-BCWP
    effort_deviation_ratio <- plan_effort_deviation/BCWP

    ## task hours estimation
    baseline_task_hours <- NoData
    project_completed_parts_plan_hours    <- BCWP
    project_completed_parts_actual_hours  <- ACWP
    growth_in_task_hours_baseline_to_plan <- NoData

    plan_date_set <- subset(ev_plan_info, task_date_key < 99999000)
    ...
    product_delivery_actual_info <- subset(time_info, regexpr("\\Test$", time_info$phase_short_name) != -1)
    ...
    plan_DevTotalratio <- plan_construction_effort/total_plan_effort
    ...
    actual_DevTotalratio <- actual_construction_effort/total_actual_effort
    ...
    production_rate_total <- actualAM/total_actual_effort
    ...
    if (length(ev_actual_complete_info$wbs_element_key) == 0) {
      ...
      dataset_effort_by_wbs <- aggregate(list
                                         (
                                             sum_plan_time   = ev_actual_complete_info$task_plan_time_minutes,
                                             sum_actual_time = ev_actual_complete_info$task_actual_time_minutes
                                         ),
                                         by = list(wbs_element_key=ev_actual_complete_info$wbs_element_key),
                                         FUN=sum, na.rm=TRUE
                                         )
